...
const Modal = forwardRef(({modalHeading, infoSubheading1, info1, infoSubheading2, info2, additionalInfo, onClose, onPayNow, modal}, ref) => {
  return (
    <section ref={ref} id='modal' tabIndex={-1} className={`w-full h-[140%] flex-col justify-center items-center absolute ${modal ? 'flex' : 'hidden'}`}>
        <div className='w-[50svw] h-fit px-10 py-5 bg-white rounded-2xl flex flex-col justify-center items-center gap-5'>
            ...
            <button type='button' className='w-full py-3 font-bold text-lg bg-orange-500 text-white rounded-2xl' onClick={onPayNow}>Pay Now</button>
        </div>
    </section>
  )
})
