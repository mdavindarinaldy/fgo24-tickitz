...
function EditMoviePage() {
  ...
  const onSubmit = async (data) => {
    ...
    setErrorPoster(validateFile(posterFile, 'Poster'))
    setErrorBackdrop(validateFile(backdropFile, 'Backdrop'))
    if (errorPoster || errorBackdrop) return
    ...
    try {
      const res = await http(currentLogin.token).put(`/admin/movie/${id}`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
      if (res.data.success) {
        reset()
        setSuccess('Berhasil update data film') 
        setTimeout(function () {
          setSuccess('')
        }, 5000)
      }
    } catch (err) {
      setError('Gagal update data film: ', err)
    }
  }
    ...
    <div>
      ...
      <div className='w-svw bg-gray-200 h-fit py-10 px-5 flex flex-col gap-10 justify-starts items-center'>
        <div className='w-[90%] lg:w-[65%] h-[95%] bg-white rounded-lg px-10 py-10 flex flex-col gap-5'>
          <span className='font-bold text-2xl'>Edit Movie</span>
          <form 
            id='updateMovie'
            ...
            className='self-center flex flex-col gap-5 w-[95%] h-fit'>
              ...
              {success && <span className='text-lg text-green-400 font-semibold'>{success}</span>}
          </form>
        </div>
      </div>
    </div>
